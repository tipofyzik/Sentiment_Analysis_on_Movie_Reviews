...
class DataPreprocessor:
    ...
    def __preprocess_data_spacy(self, texts: list[str]) -> list[str]:
        ...
        lemmatized_texts = []
        for doc in self.__nlp.pipe(
            texts, batch_size=self.__spacy_batch_size, n_process=self.__spacy_n_process
        ):
            tokens = [
                token.lemma_
                for token in doc
    ...
    def preprocess_data_batch(
        self, dataset: pd.DataFrame, column_to_preprocess: str
    ) -> pd.DataFrame:
        """
        ...
        dataset[column_to_preprocess] = dataset[column_to_preprocess].apply(
            self.__remove_noise_from_data
        )
        processed_texts = self.__lemmatize_data_batch(
            dataset[column_to_preprocess].tolist()
        )
        ...
        print(
            f"Execution time: {int(execution_time // 3600)}h {int((execution_time % 3600) // 60)}m {int(execution_time % 60)}s"
        )
